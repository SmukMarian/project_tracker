...
Подробные пошаговые инструкции для чистого окружения смотрите в [docs/USAGE.md](docs/USAGE.md): установка Python/Node-зависимостей, выбор workspace и сценарии запуска backend, фронтенда и desktop-обёртки.

## Готовый запуск
- **Сборка + backend**: после `npm run build` и старта `uvicorn backend.app:app --reload` SPA доступно по адресу http://localhost:8000/app/.
- **Dev-сценарий**: поднимите backend и откройте URL, который покажет Vite (`npm run dev`, обычно http://localhost:5173). Этот же URL можно передать в desktop-режим через `python desktop.py --ui-url ...`.

Workspace должен указывать на существующую папку с правами записи: там лежит база данных, медиафайлы, логи и обновления.
...
## Медиа и прогресс
- Обложку проекта и вложения добавляйте в диалоге создания/редактирования проекта: поле «Обложка» принимает изображение, «Вложения» — любые файлы; всё сохраняется в `workspace/media/` и отображается в карточке.
- Общий прогресс и KPI показаны в левой панели списка проектов; развёрнутый отчёт открывается кнопкой «KPI» в верхнем меню и строится по endpoint'у `/kpi`.
