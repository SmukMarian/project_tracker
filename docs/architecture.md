...
## Next steps
1. Прогонять смоук-тест обновлений (`python packaging/test_update_flow.py`) после сборки инсталлятора, чтобы подтвердить валидность манифеста и скачиваемого пакета.
